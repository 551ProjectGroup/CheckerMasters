...
guiPiece * DisplayBoard::makeScene(BoardState *currstate, QGraphicsScene *scene, MainWindow *w)
{
    addBoardScene(scene);
    ...
    for(int i=0;i<8;i++) {
        for(int j=0;j<8;j++) {
            if(!currstate->getPiece(i,j).getEmpty()) {
                guiPiece *g = new guiPiece(i,j,currstate->getPiece(i,j).getPieceColor(),currstate->getPiece(i,j).getType(),currstate,w);
                scene->addItem(g);
                count++;
...
void DisplayBoard::displayScene(BoardState *currstate, QGraphicsScene *scene, MainWindow *w)
{
    addBoardScene(scene);
    int count=0;
    for(int i=0;i<8;i++) {
